...
for tau in tau_values:
    ...
    if not load_cache:
        ...
        while not torch.all(finished):
            ...
            tick += 1
            current_num = torch.sum(occupancy, dim=1)
            print(f"tau={tau:.2f}, all sim protein_count: {current_num.shape}, global_time: {global_time}")
    ...
    data_per_tau[tau] = {
        ...
        'pop_std': pop_std,
        'times': times_tau,
        'population_counts': population_counts,  # optional
...
for tau in tau_values:
    ...
    if pad_len > 0:
        pad_array = np.full((num_sim, pad_len), num_protein)
...
ax3.set_ylabel('Squared Difference')
