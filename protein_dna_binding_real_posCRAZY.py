...
for tau_anim in tau_values:
    if tau_anim in data_per_tau:
        ...
        def animate(frame):
            ax_anim.clear()
            ax_anim.bar(np.arange(length_dna), histogram_history[frame], width=1)
        ...
        ani.save(f'protein_dna_binding_animation_tau{tau_anim:.2f}.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
