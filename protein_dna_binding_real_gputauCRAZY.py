...
tau_values = np.logspace(np.log10(10), np.log10(100), 10)
...
load_cache_all = all([os.path.exists(f"simulation_cache_tau{tau:.2f}_num{num_protein}_nsim{num_sim_per_tau}.pkl") for tau in tau_values])
if not load_cache_all:
    ...
    population_history = np.stack(population_history, axis=1)  # (total_sim, ticks)
...
for tau in tau_values:
    ...
    times_tau = np.arange(0, max_steps * dt, dt)
    data_per_tau[tau] = {
...
global_t_max = max(all_t_max)
global_max_steps = int(global_t_max / dt) + 1
